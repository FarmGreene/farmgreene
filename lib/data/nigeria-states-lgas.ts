/**
 * Nigerian states mapped to their Local Government Areas (LGAs).
 * Single source of truth for the state/LGA dependent dropdowns.
 */
export const NIGERIA_STATES_LGAS: Record<string, string[]> = {
  ...
  Jigawa: [
    ...
    "Kafin Hausa", "Kazaure", "Kiri Kasama", "Kiyawa", "Kaugama", "Maigatari",
  ...
  Niger: [
    ...
    "Mashegu", "Mokwa", "Munya", "Paikoro", "Rafi", "Rijau", "Shiroro",
...
/** Alphabetical list of all states (+ FCT) for the state dropdown. */
export const NIGERIAN_STATES: string[] = Object.keys(NIGERIA_STATES_LGAS);

/** LGAs for a given state (empty array if state is unknown/unset). */
