...
// Note: Using a simplified list for states/LGAs. In a real app, this would come from a shared util.
const STATES = ["Osun", "Oyo", "Lagos", "Ogun", "Kwara"];
...
export function Step4Location({ onSubmit, defaultValues, isLoading }: Step4LocationProps) {
  const {
    register,
    control,
    handleSubmit,
  ...
  return (
    <form id="step-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="space-y-4">
        <h4 className="text-sm font-semibold border-b pb-2">Where is it located?</h4>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="e.g. Osogbo" {...register("lga")} />
        ...
        <div className="space-y-1.5">
          <Label>Exact Address <span className="text-xs text-muted-foreground">(Only shared after booking is confirmed)</span></Label>
          <Input placeholder="e.g. 15 Farm Road" {...register("exactAddress")} />
        </div>
        
        <div className="space-y-1.5">
          <Label>Landmark</Label>
          <Input placeholder="e.g. Behind the central market" {...register("landmark")} />
        </div>
      </div>

      <div className="space-y-4 pt-4">
        <h4 className="text-sm font-semibold border-b pb-2">Delivery & Logistics</h4>
        <div className="p-4 border rounded-xl space-y-4 bg-slate-50/50 dark:bg-slate-900/20">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Can you transport this equipment to the renter?</p>
            </div>
            <Controller
      ...
      <div className="space-y-4 pt-4">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-[10px] text-muted-foreground">How many days in advance must a renter book?</p>
        </div>
            ...
            render={({ field }) => (
              <Select value={field.value || undefined} onValueChange={field.onChange}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="custom">Custom (Block specific dates)</SelectItem>
                </SelectContent>
              </Select>
            )}
          />
        ...
        {availabilityType === "custom" && (
           <div className="pt-2">
             ...
             <p className="text-[10px] text-muted-foreground mb-2">Select dates when the equipment is unavailable.</p>
             <Card className="p-2 inline-block">
               <Controller
                  ...
                  render={({ field }) => (
                    <Calendar
                      mode="multiple"
                      selected={(field.value || []).map((d: string) => new Date(d))}
